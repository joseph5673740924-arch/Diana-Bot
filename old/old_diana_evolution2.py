...
diana_evolution.py
==================
黛安娜 (Diana) 自我成長與演化腳本 (手動選擇模型與防頻率限制版 + 主動升級功能)
...
- 支援讀取 upgrade.txt 執行主動功能擴充要求，並在成功後自動更名防重複執行
...
UPGRADE_FILE = os.path.join(BASE_DIR, "upgrade.txt") # 主動要求升級指令檔
...
def evolve_code(client: genai.Client, context: dict, model_name: str) -> tuple[str, str]:
    ...
    if os.path.exists(UPGRADE_FILE):
        ...
        if upgrade_request:
            print(f"📥 偵測到 {os.path.basename(UPGRADE_FILE)}！準備執行主動功能要求...")
...
- 若決定新增功能：
  {"status": "UPDATED", "summary": "簡述本次為黛安娜新增了什麼技能或指令", "code": "完整且可執行的完整 bot.py Python 程式碼"}"""
    ...
    try:
        ...
        # 🌟 成功升級後，將 upgrade.txt 改名避免重複執行
        if upgrade_request:
            done_file = f"{UPGRADE_FILE}.done_{datetime.now().strftime('%Y%m%d_%H%M')}"
            try:
                os.rename(UPGRADE_FILE, done_file)
                print(f"🧹 主動要求已完成實作，檔案已重命名為: {os.path.basename(done_file)}")
            except Exception as e:
                print(f"⚠️ 無法重命名 {UPGRADE_FILE}: {e}")
